...
class CustomImageEventHandler : public CImageEventHandler
{
	...
	public:
		...
		CustomImageEventHandler(int id, int rec_frames, int curr_height, int curr_width, \
			bool motion_activation, bool many_files = true, FILE* file = NULL, std::string wd = NULL)
		{
			camera_number   = id; 
    	...
    	virtual void OnImageGrabbed( CInstantCamera& camera, const CGrabResultPtr& ptr_grab_result)
    	{
            ...
            {                    
				...
				//bool fly_moving = image_analyser.isMoving(M,camera_number);
            	bool fly_moving = capture_flag; 
            	if (!fly_moving)
            	{
            		fly_moving = 0.25 > (static_cast <float> (rand()) / static_cast <float> (RAND_MAX));
			...
			if(!motion_act || capture_flag)
			{
				...
				unsigned int ticks =((CBaslerUsbGrabResultPtr) ptr_grab_result)->ChunkTimestamp.GetValue();
				...
				if (direct_save)
				{
					...
					sstream << path << "/camera_" << camera_number << "_img_" << frame_count << ".png";
			...
			if( fps <= DISPLAY_FPS || index %((int)(fps/DISPLAY_FPS)) == 0)
			{
			    ...
			    if (motion_act && camera_number==0)
			    {	
                    cv::circle(M,cv::Point(30,30),15.0,annotation_color,CV_FILLED,8);
					
			    }
			
			    cv::imshow(int_string.str(),M);                              
			    cv::resizeWindow(int_string.str(), (img_width*WINDOW_HEIGHT)/img_height,WINDOW_HEIGHT);
			    cv::waitKey(1);
...
void frame_grabber::set_performance_mode(bool onOff)
{	
	try 
	{
	...
	{
		cerr << "Exeception on set_performance_mode(). Reason: "
 		 << e.GetDescription() << e.what() << endl;
	}
}

void frame_grabber::set_metadata(bool onOff)
{
	try 
	{
	...
	{
		cerr << "Exeception on set_metadata(). Reason: "
 		 << e.GetDescription() <<  e.what() << endl;
	}
	return; 
}

void frame_grabber::set_synchronization(bool trigger, bool strobe)
{
...
frame_grabber::frame_grabber()
{
	...
	try{
		...
		{	
			...
			CIntegerPtr _maxTransferSize(cameras[i].GetStreamGrabberNodeMap().GetNode("MaxTransferSize"));
			CIntegerPtr _transferPriority(cameras[i].GetStreamGrabberNodeMap().GetNode("TransferLoopThreadPriority")); 
	...
	set_synchronization(false,false);
	...
	{
		string guid = string(cameras[i].GetDeviceInfo().GetSerialNumber().c_str()); 
		cameras_setROI(defaultROI[guid.c_str()]["Width"].asInt(),defaultROI[guid.c_str()]["Height"].asInt(),
					   defaultROI[guid.c_str()]["OffX"].asInt() ,defaultROI[guid.c_str()]["OffY"].asInt(), i); 
	}

    image_analyser.setNumCams(num_devices);
	return num_devices; 
}
...
void frame_grabber::cameras_start(int numFrames)
{
	try 
	...
	{	
		...
		for (int i= 0; i<num_devices; i++)
		{
			if (numFrames)
			{
				cameras[i].StartGrabbing((size_t)numFrames,strategy,GrabLoop_ProvidedByInstantCamera); 
			}
			else 
			{
...
void frame_grabber::cameras_stop()
{
	try 
	{
		for (int i=0; i<1; i++)
...
void frame_grabber::cameras_grab(std::string path, int obs_frames, int rec_frames, bool motion_act, bool direct_save, bool trigger, bool strobe)
{	
	try
	{ 
		time_t raw_time;
		...
		{	
			...
			for (int i=0; i<num_devices;i++)
			{
				...
				new (&(grabHandlers[i])) CustomImageEventHandler(i, rec_frames ,height[i],width[i],
													   motion_act,direct_save, arhFile[i], path);
				cameras[i].RegisterImageEventHandler(&(grabHandlers[i]), RegistrationMode_Append, Cleanup_None);
			}
	
			set_performance_mode(true);
			set_metadata(true);
			set_synchronization(trigger,strobe);
			...
			for (int i=0; i<num_devices;i++)
			{
				capture_metadata["Number of Frames"][to_string(i)] = grabHandlers[i].recorded_frames;         
				...
				{
					capture_metadata["TimeStamps"]    [to_string(i)][to_string(j)]   = grabHandlers[i].getTimestamp(j);
					capture_metadata["Frame Counts"]  [to_string(i)][to_string(j)]   = grabHandlers[i].getFrameNumber(j); 
				}
			} 
			...
			if(direct_save)
			{
				create_preview(capture_metadata,path);
			}
		}
		...
		else 
		{
			...
			{	
				...
				if( cameras[i].RetrieveResult( 5000, ptr_grab_result, TimeoutHandling_ThrowException)
				...
				{	
                    ...
                    cv::imshow(int_string.str(),M);                                              
					cv::resizeWindow(int_string.str(), (width[i]*WINDOW_HEIGHT)/height[i],WINDOW_HEIGHT);
					cv::waitKey(1);
				}
				else
				{
...
void frame_grabber::cameras_save(std::string path)
{
	try
	{
		...
		for (int j=0 ; j < captureMetadata["Number of Cameras"].asInt(); j++)
		{
			...
			for (int i =0; i < captureMetadata["Number of Frames"][to_string(j)].asInt(); i++)
			{
				...
				sstream_img << path <<"/camera_" << j << "_img_"<< captureMetadata["Frame Counts"][to_string(j)][to_string(i)] << ".png";
		...
		create_preview(captureMetadata, path);
	}
	catch (const GenericException & e)
	{
...
void frame_grabber::cameras_setFPS(float new_fps, bool for_trigger)
{
	try
	{	
		if(!for_trigger)
...
void frame_grabber::cameras_setROI(unsigned int newWidth,unsigned int newHeight,
...
std::array<long int,4> frame_grabber::cameras_getROI(int i)
{
	CIntegerPtr _width (cameras[i].GetNodeMap().GetNode("Width"));
	CIntegerPtr _height(cameras[i].GetNodeMap().GetNode("Height"));
	CIntegerPtr _offX  (cameras[i].GetNodeMap().GetNode("OffsetX"));
...
string frame_grabber::cameras_getGUID(int i)
{
