...
void updateFrameView()
{
  if (!captureRunning)
  {
      cameras_grab("Not Applicable",0,0,false,false,false,false,liveStream,saveCams,streamCam);
  }
...
void start_capture()
{
  ...
  float rec_time_sec = motion_activation ? convertTime(fl_rec_time, rec_scale) : convertTime(fl_rec_time, obs_scale);
...
/////Create directory only for images
 ...
 //////Starting cameras
  ...
  {   
    ...
    cameras_grab(dirCreated,obs_frames,rec_frames, motion_activation, seperate_images, trigger,strobe,liveStream,saveCams,streamCam); 
    cameras_stop(); 
...
{
 int val = captStream[0]+captStream[1]+captStream[2]+captStream[3]+captStream[4]+captStream[5]+captStream[6];
...
void addBatch(int mode)
{
  ...
  batchFile<<dir<<","<<videofrom[0]<<","<<videofrom[1]<<","<<videofrom[2]<<","<<videofrom[3]<<","<<videofrom[4]<<","<<videofrom[5]<<","<<videofrom[6]<<","<<videofrom[7]<<std::endl;
...
/*
  ...
  button(".button_set_default_roi") -text("Save ROI setup as default") -command(setDefaultROI); 
  ...
  */ 
  ...
  checkbutton(".check_captStream0") -variable(captStream[0]) -state("disabled") -command(toggle_capt_stream);
  ...
  checkbutton(".check_captStream1") -variable(captStream[1]) -state("disabled") -command(toggle_capt_stream);
  ...
  checkbutton(".check_captStream2") -variable(captStream[2]) -state("disabled") -command(toggle_capt_stream);
  ...
  checkbutton(".check_captStream3") -variable(captStream[3]) -state("disabled") -command(toggle_capt_stream);
  ...
  checkbutton(".check_captStream4") -variable(captStream[4]) -state("disabled") -command(toggle_capt_stream);
  ...
  checkbutton(".check_captStream5") -variable(captStream[5]) -state("disabled") -command(toggle_capt_stream);
  ...
  checkbutton(".check_captStream6") -variable(captStream[6]) -state("disabled") -command(toggle_capt_stream);
  ...
  checkbutton(".check_motion_activation") -variable(motion_activation) -command(toggle_motion_activation);
...
//Stimulus labels
  ...
  textw(".currentStim") -state("disabled") -background("white") -yscrollcommand(".s set") -width(25) -height(5);
  ...
  button(".button_stimulation") -text("Upload sequence") -state("disabled") -command(new_stim_sequence); 
  grid(configure,".button_stimulation") -in(".stimulus") -column(3) -row(3);
  button(".button_editStim") -text("Edit stimulation sequence") -state("disabled") -command(editSequence); 
...
//Experiment Settings 
  ...
  grid(configure,".button_start_capture") -in(".folderdata") -columnspan(2) -column(2) -row(2);
...
//Select location
  ...
  grid(configure, ".label_working_directory_postproc") -in(".postprocess1") -column(0) -row(2); 
  entry(".entry_working_directory_postproc") -width(40) -state("disabled"); 
  grid(configure, ".entry_working_directory_postproc") -in(".postprocess1") -columnspan(3) -column(1) -row(2);
  ...
  label(".label_postproc") -text("Getting images from temp file:") -justify("left") -font("title");
  ...
  grid(configure, ".label_working_directory_postproc_images") -in(".postprocessImages") -column(0) -row(1); 
  entry(".entry_working_directory_postproc_images") -width(30); 
  grid(configure, ".entry_working_directory_postproc_images") -in(".postprocessImages") -column(1) -row(1); 
  button(".button_process_images") -text("Post Process Images") -command(process);  
  grid(configure,".button_process_images") -in(".postprocessImages") -columnspan(2) -column(0) -row(2);

  pack(".postprocessImages");

  // Putting it all together
  //grid(configure, ".roi_settings") -in(".config") -column(0) -row(0) -sticky("w") -columnspan(2);
  grid(configure, ".cameras")      -in(".config") -column(0) -row(1) -sticky("w") -columnspan(2);
  grid(configure, ".capture")      -in(".config") -column(0) -row(2) -sticky("w");
  grid(configure, ".stimulus")      -in(".config") -column(1) -row(2) -sticky("w");
  grid(configure, ".experiment")   -in(".config") -column(0) -row(3) -sticky("w") -columnspan(2);
  grid(configure, ".folderdata")   -in(".config") -column(0) -row(4) -sticky("w") -columnspan(2);
  grid(configure, ".postprocess0")  -in(".config") -column(0) -row(5) -sticky("w") -columnspan(2);
  grid(configure, ".postprocess1")  -in(".config") -column(0) -row(6) -sticky("w") -columnspan(2);
  grid(configure, ".postprocess2")  -in(".config") -column(0) -row(7) -sticky("n") -columnspan(2);
  grid(configure, ".postprocess3")  -in(".config") -column(0) -row(8) -sticky("s") -columnspan(2);
  grid(configure, ".postprocessImages")  -in(".config") -column(0) -row(9) -sticky("w") -columnspan(2);
